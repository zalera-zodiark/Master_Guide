...
import com.charros_software.master_guide.reply_app.data.Account

object LocalAccountsDataProvider {

    val allUserAccount = mutableListOf(
        Account(
            1L,
            0L,
            "Jeff",
    ...
    fun getDefaultUserAccount() = allUserAccount.first()

    // Whether or not the given [Account.id] uid is an account owned by the current user.
    fun isUserAccount(uid: Long): Boolean = allUserAccount.any { it.uid == uid }
